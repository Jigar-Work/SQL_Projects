...
select pizza_category,round(sum(total_price)) as Total_sales, concat(round((sum(total_price)/total_revenue)*100,2),"%") as Percentage from pizza_sales,
(select sum(total_price) as total_revenue from p_s_v) as divide group by pizza_category;
